...
int8_t bsp_i2c_hw_write_buf(i2c_hw_class_t *ptr, uint8_t reg, uint8_t *buf, int len)
{
    if(reg==NULL){
        ...
        if(HAL_I2C_Mem_Write(ptr->pI2C,ptr->address,reg,I2C_MEMADD_SIZE_8BIT,buf,len,1000)!=HAL_OK){
            return -1;
        }
    }
...
int8_t bsp_i2c_hw_read_buf(i2c_hw_class_t *ptr, uint8_t reg, uint8_t *buf, int len)
{
    if(reg==NULL){
        ...
        if(HAL_I2C_Mem_Read(ptr->pI2C,ptr->address,reg,I2C_MEMADD_SIZE_8BIT,buf,len,1000)!=HAL_OK){
            return -1;
        }
    }
    return 0;
